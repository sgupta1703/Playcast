...
module.exports = {
  ...
  // Resolve paths so runtime is not dependent on current working directory
  ...
  AUDIO_PRE_SECONDS: Number(process.env.AUDIO_PRE_SECONDS || 75), // 75s before play for context
  VIDEO_CLIP_PRE: Number(process.env.VIDEO_CLIP_PRE || 10), // seconds before play to start clip
  VIDEO_CLIP_LENGTH: Number(process.env.VIDEO_CLIP_LENGTH || 45), // clip duration in seconds
};
